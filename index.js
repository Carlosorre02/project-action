...
fs.readFile(reportPath, "utf8", async (err, data) => {
    ...
    try {
        ...
        const getTags = async (namespace, repository, currentTag) => {
            ...
            // Estrai la major e minor version e la parte platform dell'immagine base
            const [baseVersion, basePlatformVersion] = currentTag.split("-").map((part, index) =>
                index === 0 ? part : part.replace("alpine", ""));
            ...
            while (url) {
                try {
                    ...
                    pageTags.forEach((tag) => {
                        ...
                        const [tagVersionBase, tagPlatformVersion] = tagVersion.split("-").map((part, index) =>
                            index === 0 ? part : part.replace("alpine", ""));

                        // Filtra solo i tag che hanno la stessa versione base e piattaforma dell'immagine base
                        if (
                            tagVersionBase === baseVersion && // stessa versione di base
                            tagPlatformVersion > basePlatformVersion // versione piattaforma successiva
                        ) {
                            tags.push(tag.name);
                        }
                    });
