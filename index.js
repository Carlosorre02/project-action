...
let summaryReport = {
    ...
    iterationCount: 0,
    versionSelectionLogic: "Filtraggio dei tag in base al pattern dell'immagine base.",
    imagesAnalyzed: [],
};
...
fs.readFile(reportPath, "utf8", async (err, data) => {
    ...
    try {
        ...
        const getTags = async (namespace, repository, currentTag) => {
            ...
            const basePattern = currentTag.split(":")[1];
            const hasPatch = semver.valid(basePattern) && semver.patch(basePattern) > 0;
                ...
                try {
                    ...
                    pageTags.forEach((tag) => {
                        ...
                        if (semver.valid(tagVersion)) {
                            const matchesPattern = hasPatch 
                                ? semver.diff(basePattern, tagVersion) !== 'major'
                                : semver.diff(basePattern, tagVersion) === 'minor';
        ...
        const sortTags = (tags) => {
            return tags.sort((a, b) => semver.compare(a, b));
        };
        ...
        if (availableTags.length > 0) {
            ...
            for (const image of sortedTags) {
                core.info(`Analisi in corso per immagine: ${image}`);
                const reportFileName = `trivy-report-${image}.json`;

                const trivyScan = (image, reportFileName) => {
                ...
                try {
                    ...
                    const parseTrivyReport = () => {
                        const reportData = fs.readFileSync(reportFileName, "utf8");
