...
fs.readFile(reportPath, "utf8", async (err, data) => {
    ...
    try {
        ...
        const getTags = async (namespace, repository, currentTag) => {
            ...
            // Estrai la parte di versione e piattaforma dall'immagine base
            const [baseVersion, basePlatform] = currentTag.split(":")[1].split("-");
            ...
            while (url) {
                try {
                    ...
                    // Filtra i tag che rientrano nel formato specificato
                    pageTags.forEach((tag) => {
                        const [tagVersion, tagPlatform] = tag.name.split("-");
                        if (
                            tagPlatform &&
                            tagPlatform.startsWith(basePlatform) &&
                            semver.valid(tagVersion) &&
                            semver.gte(tagVersion, baseVersion)
                        ) {
                            tags.push(tag.name);
                        }
                    });
