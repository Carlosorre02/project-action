...
const countVulnerabilitiesBySeverity = (vulnerabilities) => {
    ...
    Object.keys(counts).forEach((severity) => {
        counts[severity] = vulnerabilities[severity] ? vulnerabilities[severity].length : 0;
    });
    return counts;
};

// Funzione per determinare l'immagine migliore in base al conteggio delle vulnerabilità
const determineBestImage = () => {
    let baseImageCounts = countVulnerabilitiesBySeverity(summaryReport.imagesAnalyzed[0].vulnerabilities);
    let bestImage = summaryReport.baseImage;
    let bestCounts = baseImageCounts;

    summaryReport.imagesAnalyzed.slice(1).forEach((image) => {
        const imageCounts = countVulnerabilitiesBySeverity(image.vulnerabilities);
        if (
            imageCounts.CRITICAL < bestCounts.CRITICAL ||
            (imageCounts.CRITICAL === bestCounts.CRITICAL && imageCounts.HIGH < bestCounts.HIGH) ||
            (imageCounts.CRITICAL === bestCounts.CRITICAL && imageCounts.HIGH === bestCounts.HIGH && imageCounts.MEDIUM < bestCounts.MEDIUM) ||
            (imageCounts.CRITICAL === bestCounts.CRITICAL && imageCounts.HIGH === bestCounts.HIGH && imageCounts.MEDIUM === bestCounts.MEDIUM && imageCounts.LOW < bestCounts.LOW)
        ) {
            bestCounts = imageCounts;
            bestImage = image.target;
        }
    });
        ...
        if (availableTags.length > 0) {
            ...
            const trivyScan = async (namespace, repository, image, reportFileName) => {
                ...
                return new Promise((resolve, reject) => {
                    exec(
                        `trivy image --format json --output ${reportFileName} --vuln-type os,library --severity LOW,MEDIUM,HIGH,CRITICAL ${fullImageName}`,
                        (error, stdout, stderr) => {
                            if (error) {
            ...
            // Determina l'immagine migliore e logga il risultato
